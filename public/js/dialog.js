...
(function () {
	...
	function createAlertDOM() {
		...
		overlay.innerHTML = `
		  <div class="absolute inset-0 app-alert-backdrop bg-black/40"></div>
	...
	overlay.addEventListener('click', (e) => {
		if (e.target === overlay || e.target.classList.contains('app-alert-backdrop')) close();
	});
	...
	function showAlert(message, options) {
		try {
			const { title = 'Bilgi', type = 'success', autoCloseMs = 0, dim = true } = options || {};
			setStyle(type);
			...
			// dim=false ise arka plan karartmayı kaldır
			const backdrop = overlay.querySelector('.app-alert-backdrop');
			if (backdrop) {
				if (dim === false) {
					backdrop.classList.remove('bg-black/40');
					backdrop.style.background = 'transparent';
				} else {
					if (!backdrop.classList.contains('bg-black/40')) backdrop.classList.add('bg-black/40');
