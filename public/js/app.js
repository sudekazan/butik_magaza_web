...
const validateImageUrl = (imageUrl, base64ImageUrl = null) => {
  ...
  // Üçüncü öncelik: Normal imageUrl
...
const loadFeaturedProducts = async () => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Öne çıkan ürünler alınamadı: ${response.status}`);
    }
    ...
    if (featuredData && featuredData.length > 0) {
      ...
      console.log('ℹ️ Öne çıkan ürün bulunamadı');
...
const showCampaignProducts = async () => {
  try {
    // Ürünler henüz yüklenmediyse önce yükle
    if (!allProducts || allProducts.length === 0) {
      await fetchAndRenderProducts('', false, null);
    }
    ...
    console.error('❌ Kampanya ürünleri gösterilirken hata:', error);
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Kampanya CTA butonu: Kampanya ürünlerini göster
  const campaignCTAButton = document.getElementById('campaignCTAButton');
  campaignCTAButton?.addEventListener('click', async () => {
    // currentCampaign varsa kategori sayfasına yönlendir
    if (currentCampaign && currentCampaign._id) {
      window.location.href = `/category?campaignId=${currentCampaign._id}`;
      return;
    }
    // Fallback: kampanya ürünlerini ana sayfada göster
    await showCampaignProducts();
  });

  // Üst kampanya barına tıklanınca da kampanya ürünlerini göster
  const activeCampaignBar = document.getElementById('activeCampaignBar');
  activeCampaignBar?.addEventListener('click', async () => {
    if (currentCampaign && currentCampaign._id) {
      window.location.href = `/category?campaignId=${currentCampaign._id}`;
      return;
    }
