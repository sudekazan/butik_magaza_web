      ...
      catList.querySelectorAll('.cat-del').forEach((btn) =>
        btn.addEventListener('click', async () => {
          ...
          try {
            ...
            if (result.success) {
              showAlertDialog(`Kategori başarıyla silindi!\n${result.message}`, { type: 'success', title: 'Başarılı', dim: false, autoCloseMs: 2000 });
            } else {
...
const openNewProductModal = (productData) => {
  ...
  createBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      showAlertDialog('Yeni ürün başarıyla oluşturuldu!', { type: 'success', title: 'Başarılı', dim: false, autoCloseMs: 2000 });
      closeModal();
      await loadProducts();
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  prodForm?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      try {
        ...
        showAlertDialog(editingProductId ? 'Ürün başarıyla güncellendi!' : 'Ürün başarıyla eklendi!', { type: 'success', title: 'Başarılı', dim: false, autoCloseMs: 2000 });
        editingProductId = null;
        await loadProducts();
  ...
  const setupBulkDeleteSystem = () => {
    ...
    bulkDeleteBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        showAlertDialog(`${data.deletedCount} ürün başarıyla silindi!`, { type: 'success', title: 'Başarılı', dim: false, autoCloseMs: 2000 });
  ...
  const deleteCampaign = async (campaignId) => {
    ...
    try {
      ...
      if (res.ok) {
        showAlertDialog('Kampanya başarıyla silindi!', { type: 'success', title: 'Başarılı', dim: false, autoCloseMs: 2000 });
        loadCampaigns();
      } else {
        const error = await res.json();
        alert(`Hata: ${error.message}`);
