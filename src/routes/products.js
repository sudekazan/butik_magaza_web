...
import { saveImageBuffer, deleteImageByUrl } from '../config/storage.js';
  ...
  try {
    ...
    if (req.files.images && req.files.images.length > 0) {
      ...
      for (let i = 0; i < req.files.images.length; i++) {
        ...
        const stored = await saveImageBuffer({ buffer, originalName: file.originalname, contentType: file.mimetype });
        storedResults.push({ stored, originalname: file.originalname });
      }
      images = storedResults.map((r, index) => ({
        url: r.stored.url,
        alt: r.originalname || '',
        isMain: index === 0,
        size: r.stored.size || 0,
        mimeType: r.stored.mimeType || '',
        uploadedAt: r.stored.uploadedAt || new Date()
      }));
        ...
        if (Array.isArray(parsedVariants)) {
          ...
          processedVariants = parsedVariants.map((variant, variantIndex) => {
            ...
            if (req.files.variantImages && Array.isArray(req.files.variantImages)) {
              ...
              if (take > 0) {
                const pickedFiles = req.files.variantImages.slice(variantImageIndex, variantImageIndex + take);
                const uploaded = [];
                for (const f of pickedFiles) {
                  const b = fs.readFileSync(f.path);
                  const st = await saveImageBuffer({ buffer: b, originalName: f.originalname, contentType: f.mimetype });
                  uploaded.push(st.url);
                }
                processedVariant.images = uploaded;
    ...
    if (processedVariants.length > 0) {
      for (const variant of processedVariants) {
        ...
        if (variant.images && variant.images.length > 0) {
          // Base64 üretimi kaldırıldı (dosya yolu/URL saklanıyor)
        }
        ...
        const variantProduct = new Product({
          ...
          imageUrl: variant.images[0] ? variant.images[0] : created.imageUrl,
          images: variant.images && variant.images.length > 0 ? variant.images.map(imgUrl => ({
            url: imgUrl,
            alt: `${variant.color} variant image`,
            isMain: false
          })) : [],
          // Base64 alanları geriye dönük için boş bırakılıyor
          base64ImageUrl: '',
          base64Images: [],
          stock: variant.stock,
          sizeStocks: variant.sizeStocks || [],
